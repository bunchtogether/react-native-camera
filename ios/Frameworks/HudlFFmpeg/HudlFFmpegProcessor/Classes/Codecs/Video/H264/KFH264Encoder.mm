...
- (instancetype)initWithBitrate:(int)bitrate width:(int)width height:(int)height directory:(NSString *)directory
{
    if (self = [super initWithBitrate:bitrate])
    {
        ...
        _encoder = [AVEncoder encoderForHeight:height andWidth:width bitrate:bitrate directory:directory];
        __weak KFH264Encoder *weakSelf = self;
        [_encoder encodeWithBlock:^int(NSArray* dataArray, CMTimeValue ptsValue) {
            [weakSelf incomingVideoFrames:dataArray ptsValue:ptsValue];
            return 0;
...
- (void)writeVideoFrames:(NSArray*)frames pts:(CMTime)pts
{
    ...
    for (NSData *data in totalFrames)
    {
        ...
        {
            hasKeyframe = YES;
            NSMutableData *IDRData = [NSMutableData new];
            if (!ppsWritten)
            {
                [IDRData appendData:_videoSPSandPPS];
                ppsWritten = YES;
            }
            if (sei)
            {
