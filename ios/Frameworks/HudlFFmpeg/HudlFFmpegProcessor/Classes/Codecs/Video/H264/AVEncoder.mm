...
+ (AVEncoder *)encoderForHeight:(int) height andWidth:(int) width bitrate:(int)bitrate directory:(NSString *)directory
{
    ...
    [enc initForHeight:height andWidth:width bitrate:bitrate directory:directory];
    return enc;
}

- (NSString *)makeFilename
{
    NSString* path =[[Utilities applicationSupportDirectory] stringByAppendingPathComponent:[_directory stringByAppendingPathComponent:[NSString stringWithFormat:@"capture%d.mp4", _currentFile]]];
    return path;
}

- (void)initForHeight:(int)height andWidth:(int)width bitrate:(int)bitrate directory:(NSString *)directory
{
    _directory = directory;
    _height = height;
    _width = width;
    _bitrate = bitrate;
    NSString* path = [[Utilities applicationSupportDirectory] stringByAppendingPathComponent:[directory stringByAppendingPathComponent:@"params.mp4"]];
    _headerWriter = [VideoEncoder encoderForPath:path height:height width:width bitrate:self.bitrate];
    _times = [NSMutableArray arrayWithCapacity:10];
    
    // swap between 3 filenames
    _currentFile = 1;
    _writer = [VideoEncoder encoderForPath:[self makeFilename] height:height width:width bitrate:self.bitrate];
}
...
- (void) onParamsCompletion
{
    ...
    if ([self parseParams:_headerWriter.path])
    {
        ...
        __weak AVEncoder *weakSelf = self;
        dispatch_source_set_event_handler(_readSource, ^{
            [weakSelf onFileUpdate];
        });
...
- (void)onEncodedFrame
{
    ...
    @synchronized(_times)
    {
        ...
        {
            //DDLogVerbose(@"no pts for buffer");
        }
    }
    if (_outputBlock != nil)
    {
