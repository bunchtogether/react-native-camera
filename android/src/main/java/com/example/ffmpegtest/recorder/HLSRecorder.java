...
public class HLSRecorder {
    ...
    private int totalFrameCount = 0;											// Used to calculate realized FPS
    ...
    private int ADTS_LENGTH = 7;										// ADTS Header length

    private ByteBuffer videoSPSandPPS;									// Store the SPS and PPS generated by MediaCodec for bundling with each keyframe


    // Recording state
    private long startWhen;
    private boolean fullStopReceived = false;
    private boolean videoEncoderStopped = false;			// these variables keep track of global recording state. They are not toggled during chunking
    private boolean audioEncoderStopped = false;

    // Synchronization
    private final Object sync = new Object();				// Synchronize access to muxer across Audio and Video encoding threads

    // FFmpegWrapper
    private FFmpegWrapper ffmpeg = new FFmpegWrapper();		// Used to Mux encoded audio and video output from MediaCodec
    ...
    private void sendDataToEncoder(MediaCodec encoder, long nanoTime, byte[] bytes, int numBytes, boolean endOfStream) {
