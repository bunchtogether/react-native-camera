...
@implementation EncodeAAC

- (instancetype)init {
    
    if ( self = [super init]) {
        encodeQueue = dispatch_queue_create("audioEncodeQueue", DISPATCH_QUEUE_SERIAL);
        outputFileSocket = [[GCDAsyncUdpSocket alloc] initWithDelegate:self delegateQueue:dispatch_queue_create("audioSocketQueue", DISPATCH_QUEUE_SERIAL)];
    }
    return self;
}
        ...
        dispatch_async(encodeQueue, ^{
            ...
            if (status != noErr) {
                ...
                if (status == noErr) {
                    ...
                    [outputFileSocket sendData:fullData toHost:@"10.0.1.16" port:13339 withTimeout:-1 tag:0];
                    fullData = nil;
                    rawAAC = nil;
                } else {
                    NSLog(@"STATUS PROBLEM");
                }
                CFRelease(sampleBuffer);
...
- (void)udpSocket:(GCDAsyncUdpSocket *)sock didNotConnect:(NSError * _Nullable)error {
    NSLog(@"Did not connect to socket");
    if (error) {
        NSLog(@"%s: %@", __func__, error);
        return;
    }
